...
function getHistoricalModeNote(mode) {
  switch (mode) {
    case "TIMESTAMP_BASED":
      return "Explorer rows include timestamp data. Windows are calculated directly from transaction timestamps. Stake Tx Count is transaction count; Net Stake Delta is DACC amount change.";
    case "BLOCK_TIMESTAMP_FALLBACK":
      return "Explorer rows did not fully include timestamp data. The checker resolved block timestamps through RPC. Stake Tx Count is transaction count; Net Stake Delta is DACC amount change.";
    case "BLOCK_TIMESTAMP_FALLBACK_LIMITED":
      return "Explorer rows did not fully include timestamp data. Only the most recent block timestamps were resolved to avoid excessive RPC requests. Stake Tx Count is transaction count; Net Stake Delta is DACC amount change.";
  ...
  if (!historicalActivity || !historicalActivity.windows) {
    ...
    if (el.historicalNote) {
      el.historicalNote.textContent =
        "Historical windows are calculated from explorer timestamps when available. Stake Tx Count shows transaction count, while Net Stake Delta shows DACC amount change.";
    }
      ...
      return `
        <article class="history-window-card">
          ...
          <div class="history-stake-line">
            Net Stake Delta: <strong>${formatNullableNumber(window.netStakeFlow, 4)} DACC</strong><br>
