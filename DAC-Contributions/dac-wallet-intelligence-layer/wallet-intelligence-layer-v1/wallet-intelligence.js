...
function scoreWalletMaturity(metrics) {
  ...
  let earlyBonus = 0;
  let consistency = 0;
  let rule = "walletAgeDays unavailable";

  if (age !== null && age !== undefined) {
    if (age >= 90) {
      ageScore = 8;
      earlyBonus = 4;
      rule = "walletAgeDays >= 90";
    } else if (age >= 60) {
      ageScore = 7;
      earlyBonus = 4;
      rule = "walletAgeDays >= 60";
    } else if (age >= 30) {
      ageScore = 6;
      earlyBonus = 3;
      rule = "walletAgeDays >= 30";
    } else if (age >= 14) {
      ageScore = 4;
      earlyBonus = 2;
      rule = "walletAgeDays >= 14";
    } else if (age >= 7) {
      ageScore = 2;
      earlyBonus = 1;
      rule = "walletAgeDays >= 7";
    } else {
      ageScore = 1;
      earlyBonus = 0;
      rule = "walletAgeDays < 7";
  ...
  if (age >= 30 && metrics.activeWeeks >= 4) {
    consistency = 3;
  } else if (age >= 14 && metrics.activeWeeks >= 2) {
    consistency = 2;
  } else if (age >= 7 && metrics.activeDays >= 3) {
    consistency = 1;
  }
  ...
  return {
    ...
    points: Math.min(15, ageScore + earlyBonus + consistency),
    maxPoints: 15,
    rule,
    note:
      "Special early-participation weight. Rewards older DAC Testnet wallets while still considering activity consistency.",
  };
}
...
function scoreActivityBurst(metrics) {
  ...
  let points = 1;
  let rule = "burstRatio > 60%";

  if (metrics.totalTransactions < 5) {
    points = 5;
    rule = "totalTransactions < 5";
  } else if (ratio <= 0.25) {
    points = 10;
    rule = "burstRatio <= 25%";
  } else if (ratio <= 0.4) {
...
function scoreInteractionDiversity(metrics) {
  ...
  if (combined >= 10) {
    points = 15;
    rule = "uniqueContracts + contractCreations >= 10";
  } else if (combined >= 5) {
    points = 11;
    rule = "uniqueContracts + contractCreations >= 5";
  } else if (combined >= 3) {
    points = 7;
    rule = "uniqueContracts + contractCreations >= 3";
  } else if (combined >= 1) {
    points = 3;
    rule = "uniqueContracts + contractCreations >= 1";
  }
...
function scoreOfficialDacCommitment(metrics) {
  ...
  return {
    ...
    name: "Official DAC Commitment",
    points,
    maxPoints: 15,
    rule,
    note:
      "Uses DAC Inception Rank and DACC staking as risk reducers, not as official eligibility claims.",
  };
}
...
function renderFirstTransaction(firstTransaction) {
  ...
  if (el.firstTransactionMetaValue) {
    ...
    el.firstTransactionMetaValue.textContent =
      `${date} · ${block} · ${firstTransaction.type} · ${hash}`;
  }
}
