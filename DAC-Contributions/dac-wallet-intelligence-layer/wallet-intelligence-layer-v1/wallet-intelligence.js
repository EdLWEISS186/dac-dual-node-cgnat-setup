...
function getHistoricalModeNote(mode) {
  switch (mode) {
    case "TIMESTAMP_BASED":
      return "Explorer rows include timestamp data. Windows are calculated directly from transaction timestamps.";
    case "BLOCK_TIMESTAMP_FALLBACK":
      return "Explorer rows did not fully include timestamp data. The checker resolved block timestamps through RPC.";
    case "BLOCK_TIMESTAMP_FALLBACK_LIMITED":
      return "Explorer rows did not fully include timestamp data. Only the most recent block timestamps were resolved to avoid excessive RPC requests.";
  ...
  if (!historicalActivity || !historicalActivity.windows) {
    ...
    if (el.historicalNote) {
      el.historicalNote.textContent =
        "Historical windows are calculated from explorer timestamps when available. If timestamps are unavailable, the checker attempts a limited block timestamp fallback.";
    }
      ...
      return `
        <article class="history-window-card">
          ...
          <div class="history-stake-line">
            ...
            In: ${formatNullableNumber(window.stakeIn, 4)} · Out: ${formatNullableNumber(window.unstakeOut, 4)}
          </div>
        </article>
      `;
    })
