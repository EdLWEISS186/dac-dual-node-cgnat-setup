...
const SCORING_POLICY = Object.freeze({
  version: "WIL-v1.5.0",
  policyId: "WIL-2026-05-v1.5.0",
  status: "LOCKED",
  model: "wallet-quality-scoring-v1.5.0",
...
function renderSybilHeuristics(sybilHeuristics) {
  ...
  if (!sybilHeuristics || sybilHeuristics.status !== "READY") {
    if (el.sybilModeLabel) {
      el.sybilModeLabel.textContent = sybilHeuristics
        ? `${sybilHeuristics.version || "EOH-v1.5.0"} · ${sybilHeuristics.confidence || "LOW"}`
        : "EOH-v1.5.0";
    }
  ...
  el.sybilMetricGrid.innerHTML = [
    metricCard("Wallet Age", renderAgeCompact(metrics.walletAgeDays, sybilHeuristics.firstTransaction), "First seen on DAC"),
...
function renderScoringPolicy(policy) {
  ...
  if (el.scoringPolicyLabel) {
    el.scoringPolicyLabel.textContent = safePolicy.version || "WIL-v1.5.0";
  }

  if (el.policyIdValue) {
    el.policyIdValue.textContent = safePolicy.policyId || "WIL-2026-05-v1.5.0";
  }
  ...
  if (el.policyEngineValue) {
    el.policyEngineValue.textContent = safePolicy.model || "wallet-quality-scoring-v1.5.0";
  }
}
...
function renderScoringBreakdown(reputationScoring) {
  ...
  if (el.scoreExplanation) {
    el.scoreExplanation.textContent =
      "This score is generated under the locked WIL-v1.5.0 wallet-quality policy. Reputation scoring remains a 100-point layer, while Explorer-only Sybil Heuristics adds a separate 100-point pattern-health layer. The labels and thresholds are community-defined and are not official DAC eligibility criteria.";
  }
}
