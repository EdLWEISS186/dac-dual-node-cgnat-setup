...
const SCORING_POLICY = Object.freeze({
  version: "WIL-v1.3.0",
  policyId: "WIL-2026-05-v1.3.0",
  status: "LOCKED",
  model: "rank-aware-reputation-scoring-v1.3.0",
  maxScore: 100,
  ...
  note:
    "Community-defined scoring heuristic. DAC Inception Rank is treated as a strong participation signal, but this is not an official DAC reputation, eligibility, or Sybil system.",
});
...
function buildKnownCollectionSignals(nftCollections) {
  ...
  return {
    registryVersion: "known-collection-registry-v1.3.0",
    source: "Community-maintained known collection registry",
    scoringEffect: "reputation-score-component",
    note:
      "DAC Inception Rank is included as a 30-point component in the locked WIL-v1.3.0 reputation score.",
    matched,
  };
}
...
function renderKnownCollectionRegistry(registry) {
  ...
  if (el.knownRegistryStatusLabel) {
    el.knownRegistryStatusLabel.textContent = registry
      ? registry.registryVersion || "known-collection-registry-v1.3.0"
      : "Community registry v1.3.0";
  }
  ...
  if (el.rankDetectionDetail) {
    if (signal && signal.detected) {
      el.rankDetectionDetail.textContent = `${signal.holdings} RANK badge holding(s) found. Inferred rank: ${signal.inferredRank}. Rank score: ${signal.rankScore}/30.`;
    } else if (registry) {
...
function renderScoringPolicy(policy) {
  ...
  if (el.scoringPolicyLabel) {
    el.scoringPolicyLabel.textContent = safePolicy.version || "WIL-v1.3.0";
  }

  if (el.policyIdValue) {
    el.policyIdValue.textContent = safePolicy.policyId || "WIL-2026-05-v1.3.0";
  }
  ...
  if (el.policyEngineValue) {
    el.policyEngineValue.textContent = safePolicy.model || "rank-aware-reputation-scoring-v1.3.0";
  }
}
