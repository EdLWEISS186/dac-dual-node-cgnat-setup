...
const INTELLIGENCE_BADGE_VERSION = "DIB-v2.0.0";
const INTELLIGENCE_BADGE_NAME = "DAC Wallet Intelligence Badge";
const INTELLIGENCE_BADGE_SYMBOL = "DWIB";
...
function buildDynamicIntelligenceBadge(output) {
  ...
  const metadata = {
    ...
    description:
      "Community-built DAC Wallet Intelligence Badge. This badge represents the latest profile generated by the DAC Wallet Intelligence Layer and is intended to be user-triggered/updatable after a fresh wallet check.",
    image: "ipfs://TO_BE_DEFINED_AFTER_BADGE_ART",
...
function renderDynamicIntelligenceBadge(badge) {
  ...
  el.badgeMintButton.onclick = () => openBadgeMintRoute(badge);
  el.badgeMetadataOutput.textContent = JSON.stringify(badge.metadata, null, 2);
  el.badgeMintNote.textContent =
    "Clicking Mint prepares this badge draft locally and opens the DAC Sender NFT Launchpad route. The checker itself still does not request wallet connection or signature.";
}

function openBadgeMintRoute(badge) {
  try {
    localStorage.setItem("dacWalletIntelligenceBadgeDraft", JSON.stringify(badge));
  } catch (error) {
    console.warn("Unable to persist badge draft:", error);
  }

  const params = new URLSearchParams({
    source: "wallet-intelligence-layer",
    badge: "dynamic-intelligence",
    version: badge.version,
    wallet: badge.wallet,
    class: badge.mainBadgeClass,
    patternHealth: badge.secondary.patternHealth === null ? "" : String(badge.secondary.patternHealth),
    sybilRisk: badge.secondary.sybilRisk,
    inceptionRank: badge.secondary.inceptionRank,
    archetype: badge.secondary.archetype,
  });

  window.open(`${DAC_SENDER_NFT_LAUNCHPAD_URL}?${params.toString()}`, "_blank", "noopener,noreferrer");
}
