...
function buildReputationScore({
  ...
  return {
    ...
    scoringPolicy: {
      version: "WIL-v1.1.0",
      model: "transparent-reputation-scoring-v1",
      maxScore: 100,
      note:
        "Community-defined scoring heuristic. Not an official DAC reputation or Sybil system.",
    },
    breakdown,
  };
}
...
function renderScoringBreakdown(reputationScoring) {
  ...
  if (el.scoringPolicyLabel && reputationScoring.scoringPolicy) {
    el.scoringPolicyLabel.textContent = reputationScoring.scoringPolicy.version || "WIL-v1.1.0";
  }
  ...
  if (el.scoreExplanation) {
    el.scoreExplanation.textContent =
      "This score is generated from four transparent components: transaction activity, NFT collection diversity, NFT holdings, and native balance. The labels and thresholds are community-defined and are not official DAC eligibility criteria.";
  }
}
...
function renderScoringUnavailable(reason) {
  ...
  if (el.scoringPolicyLabel) {
    el.scoringPolicyLabel.textContent = "Unavailable";
  }
...
function resetView() {
  ...
  if (el.scoringPolicyLabel) {
    el.scoringPolicyLabel.textContent = "WIL-v1.1.0";
  }
  if (el.scoreExplanation) {
    el.scoreExplanation.textContent =
