...
const SCORING_POLICY = Object.freeze({
  version: "WIL-v1.6.0",
  policyId: "WIL-2026-05-v1.6.0",
  status: "LOCKED",
  model: "wallet-quality-scoring-v1.6.0",
  ...
  note:
    "Community-defined wallet-quality policy. v1.6.0 covers reputation scoring, Sybil Heuristics, score explainability, and risk reducer/amplifier audit output. This is not an official DAC reputation, eligibility, or Sybil system.",
});
...
const el = {
  ...
  sybilAuditGrid: document.getElementById("sybilAuditGrid"),
...
async function getSybilHeuristicsFromExplorer(address, outputContext = null) {
  ...
  if (!rawTxs.length) {
    return {
      ...
      audit: {
        whyThisScore: ["No explorer transaction rows were returned."],
        archetypeReason: "The wallet has no usable explorer transaction history for behavior analysis.",
        riskReducers: [],
        riskAmplifiers: ["No transaction history available for verification."],
  ...
  const audit = buildSybilAudit(metrics, components, patternHealthScore, sybilRiskEstimate, activityArchetype);
  const explanationFlags = audit.explanationFlags;
...
function buildSybilAudit(metrics, components, patternHealthScore, sybilRiskEstimate, activityArchetype) {
  const componentMap = Object.fromEntries(components.map((item) => [item.key, item]));

  const riskReducers = [];
  const riskAmplifiers = [];

  if (metrics.walletAgeDays !== null && metrics.walletAgeDays >= 30) {
    riskReducers.push(`Wallet has ${metrics.walletAgeDays} day(s) of DAC Testnet age.`);
  }

  if (metrics.activeDays >= 10 && metrics.activeWeeks >= 2) {
    riskReducers.push(`Activity is distributed across ${metrics.activeDays} day(s) and ${metrics.activeWeeks} week(s).`);
  }

  if (metrics.contractCreationCount > 0) {
    riskReducers.push(`${metrics.contractCreationCount} contract creation transaction(s) detected.`);
  }

  if (componentMap.builderTesterSignal && componentMap.builderTesterSignal.points >= 15) {
    riskReducers.push("Strong builder/tester signal reduced repetitive-activity risk.");
  }

  if (metrics.rankBadgeCount > 0) {
    riskReducers.push(`${metrics.rankBadgeCount} DAC Inception RANK badge(s) detected.`);
  }

  if (metrics.stakedDacc > 0) {
    riskReducers.push(`${formatNumber(metrics.stakedDacc, 4)} DACC estimated current stake detected.`);
  }

  if (metrics.burstRatio > 0.6) {
    riskAmplifiers.push(`High activity concentration: largest day represents ${formatPercent(metrics.burstRatio)} of total transactions.`);
  }

  if (metrics.uniqueCounterparties <= 2 && metrics.nativeTransferTxCount > 0) {
    riskAmplifiers.push("Low counterparty diversity in native transfer activity.");
  }

  if (metrics.topCounterpartyShare > 0.6) {
    riskAmplifiers.push(`Top counterparty concentration is ${formatPercent(metrics.topCounterpartyShare)}.`);
  }

  if (metrics.walletAgeDays !== null && metrics.walletAgeDays < 7 && metrics.totalTransactions >= 20) {
    riskAmplifiers.push("Fresh wallet with relatively high early activity.");
  }

  if (metrics.uniqueContracts === 0 && metrics.contractCreationCount === 0) {
    riskAmplifiers.push("No contract interaction or deployment path detected.");
  }

  const strongestComponents = [...components]
    .sort((a, b) => (b.points / b.maxPoints) - (a.points / a.maxPoints))
    .slice(0, 3)
    .map((item) => `${item.name}: ${item.points}/${item.maxPoints}`);

  const weakestComponents = [...components]
    .sort((a, b) => (a.points / a.maxPoints) - (b.points / b.maxPoints))
    .slice(0, 2)
    .map((item) => `${item.name}: ${item.points}/${item.maxPoints}`);

  const whyThisScore = [
    `Pattern Health is ${patternHealthScore}/100, mapped to ${sybilRiskEstimate} Sybil Risk Estimate.`,
    `Strongest components: ${strongestComponents.join(", ")}.`,
    `Lowest components to review: ${weakestComponents.join(", ")}.`,
  ];

  return {
    whyThisScore,
    archetypeReason: explainActivityArchetype(metrics, componentMap, patternHealthScore, activityArchetype),
    riskReducers,
    riskAmplifiers,
    explanationFlags: [
      `Activity archetype: ${activityArchetype}.`,
      ...whyThisScore,
      ...riskReducers.map((item) => `Reducer: ${item}`),
      ...riskAmplifiers.map((item) => `Amplifier: ${item}`),
    ],
  };
}

function explainActivityArchetype(metrics, componentMap, patternHealthScore, activityArchetype) {
  switch (activityArchetype) {
    case "INFRA STRESS TESTER / BUILDER":
      return "Classified as Infra Stress Tester / Builder because the wallet combines high activity with builder/tester evidence such as contract creation, broad recipient spread, or strong gas usage.";
    case "ECOSYSTEM PARTICIPANT":
      return "Classified as Ecosystem Participant because DAC ecosystem commitment and activity spread are both visible.";
    case "LOW-DATA NEW WALLET":
      return "Classified as Low-Data New Wallet because the wallet is recent and has limited explorer-visible activity.";
    case "REPETITIVE TRANSFER WALLET":
      return "Classified as Repetitive Transfer Wallet because burst activity, low counterparty diversity, weak builder/tester signal, and weak DAC ecosystem commitment appear together.";
    case "HEALTHY TESTNET PARTICIPANT":
      return "Classified as Healthy Testnet Participant because the Pattern Health Score is high across multiple independent components.";
    default:
      return "Classified as Mixed Pattern because the wallet has both constructive and weak signals, without one dominant behavior profile.";
  }
}

function renderSybilAudit(audit) {
  if (!el.sybilAuditGrid) return;

  if (!audit) {
    el.sybilAuditGrid.innerHTML = `
      <article class="sybil-audit-card">
        <span>Why This Score?</span>
        <p>Run a wallet check to generate score rationale.</p>
      </article>
      <article class="sybil-audit-card">
        <span>Archetype Reason</span>
        <p>Run a wallet check to explain the behavior classification.</p>
      </article>
      <article class="sybil-audit-card reducer">
        <span>Risk Reducers</span>
        <p>Constructive signals will appear here.</p>
      </article>
      <article class="sybil-audit-card amplifier">
        <span>Risk Amplifiers</span>
        <p>Risk signals will appear here.</p>
      </article>
    `;
    return;
  }
  ...
  el.sybilAuditGrid.innerHTML = `
    <article class="sybil-audit-card">
      <span>Why This Score?</span>
      ${renderAuditList(audit.whyThisScore)}
    </article>
    <article class="sybil-audit-card">
      <span>Archetype Reason</span>
      <p>${escapeHtml(audit.archetypeReason || "No archetype reason available.")}</p>
    </article>
    <article class="sybil-audit-card reducer">
      <span>Risk Reducers</span>
      ${renderAuditList(audit.riskReducers, "No strong reducer detected.")}
    </article>
    <article class="sybil-audit-card amplifier">
      <span>Risk Amplifiers</span>
      ${renderAuditList(audit.riskAmplifiers, "No major amplifier detected.")}
    </article>
  `;
}

function renderAuditList(items, emptyText = "No audit details available.") {
...
function renderSybilHeuristics(sybilHeuristics) {
  ...
  if (!sybilHeuristics || sybilHeuristics.status !== "READY") {
    if (el.sybilModeLabel) {
      el.sybilModeLabel.textContent = sybilHeuristics
        ? `${sybilHeuristics.version || "EOH-v1.6.0"} · ${sybilHeuristics.confidence || "LOW"}`
        : "EOH-v1.6.0";
    }
    ...
    renderSybilAudit(sybilHeuristics ? sybilHeuristics.audit : null);
      ...
      .map((item) => {
        ...
        return `
          <article class="sybil-component-card">
            ...
            <small>Rule: ${escapeHtml(item.rule)}<br>Why: ${escapeHtml(item.note)}</small>
  ...
  renderSybilAudit(sybilHeuristics.audit);
...
function renderScoringPolicy(policy) {
  ...
  if (el.scoringPolicyLabel) {
    el.scoringPolicyLabel.textContent = safePolicy.version || "WIL-v1.6.0";
  }

  if (el.policyIdValue) {
    el.policyIdValue.textContent = safePolicy.policyId || "WIL-2026-05-v1.6.0";
  }
  ...
  if (el.policyEngineValue) {
    el.policyEngineValue.textContent = safePolicy.model || "wallet-quality-scoring-v1.6.0";
  }
}



function explainReputationComponent(item) {
  switch (item.key) {
    case "transactionScore":
      return "Rewards total explorer-indexed transaction activity.";
    case "nftDiversityScore":
      return "Rewards collection diversity instead of only raw NFT count.";
    case "nftHoldingsScore":
      return "Rewards total NFT holdings visible through explorer token data.";
    case "nativeBalanceScore":
      return "Rewards available native DACC balance as a liquidity/readiness signal.";
    case "daccStakeScore":
      return "Rewards estimated current DACC stake as a wallet commitment signal.";
    case "dacInceptionRankScore":
      return "Rewards DAC Inception Rank badge count as a rank/QE progress signal.";
    default:
      return "Uses the matched policy rule for this component.";
  }
}
...
function renderScoringBreakdown(reputationScoring) {
      ...
      return `
        <article class="breakdown-item">
          ...
          <div class="breakdown-condition">
            ...
            Rule: <code>${escapeHtml(item.condition)}</code><br>
            Why: ${escapeHtml(explainReputationComponent(item))}
          </div>
  ...
  if (el.scoreExplanation) {
    el.scoreExplanation.textContent =
      "This score is generated under the locked WIL-v1.6.0 wallet-quality policy. Reputation scoring remains a 100-point layer, while Explorer-only Sybil Heuristics adds a separate 100-point pattern-health layer. The labels and thresholds are community-defined and are not official DAC eligibility criteria.";
  }
}
