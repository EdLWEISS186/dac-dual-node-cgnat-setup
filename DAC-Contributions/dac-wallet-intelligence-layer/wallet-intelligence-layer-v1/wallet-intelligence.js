...
// Client-side version for GitHub Pages.
// No wallet connection, no private key, no signing.
// ======================================================
...
function buildActivityAnalytics(txCount, nftTransfers, totalCollections, totalNFTs) {
  ...
  const nftParticipationRatio =
    txCount > 0 ? (nftTransfers / txCount).toFixed(2) : "0.00";
...
function renderPartialOutput(output) {
  ...
  fillUnavailableModules("Not generated due to partial explorer data.");
  renderCollections(output.nftCollections || []);
  renderJson(output);
}
...
function renderRpcFallbackOutput(output) {
  ...
  fillUnavailableModules("Not generated in RPC fallback mode.");
  renderCollections([]);
  renderJson(output);
}
...
function renderFailureOutput(output) {
  ...
  fillUnavailableModules("No verified data. Retry in a few moments.");
  renderCollections([]);
  renderJson(output);
}
...
function fillUnavailableModules(reason) {
  ...
  el.reputationScoreValue.textContent = "Not generated";
...
function renderCollections(collections) {
  if (!collections.length) {
    ...
    el.collectionsTableBody.innerHTML =
      '<tr><td colspan="5" class="empty-cell">No NFT collection data available.</td></tr>';
    return;
  }

  el.collectionCountLabel.textContent = `${collections.length} collection(s)`;

  el.collectionsTableBody.innerHTML = collections
    .map(
      (item, index) => `
        <tr>
          <td>${index + 1}</td>
          <td>${escapeHtml(item.collection || "Unknown")}</td>
          <td>${escapeHtml(item.symbol || "UNKNOWN")}</td>
          <td>${formatInteger(item.amount || 0)}</td>
          <td><code>${escapeHtml(item.contract || "")}</code></td>
        </tr>
      `
    )
    .join("");
