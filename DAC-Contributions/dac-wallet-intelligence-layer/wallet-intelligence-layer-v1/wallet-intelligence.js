...
const SCORING_POLICY = Object.freeze({
  version: "WIL-v1.3.1",
  policyId: "WIL-2026-05-v1.3.1",
  status: "LOCKED",
  model: "rank-aware-reputation-scoring-v1.3.1",
  maxScore: 100,
  components: Object.freeze({
    transactionScore: Object.freeze({
      maxPoints: 30,
      thresholds: Object.freeze([
        { condition: "txCount >= 1000", points: 30 },
        { condition: "txCount >= 500", points: 22 },
        { condition: "txCount >= 100", points: 15 },
        { condition: "txCount < 100", points: 7 },
      ]),
    }),
    nftDiversityScore: Object.freeze({
      maxPoints: 15,
      thresholds: Object.freeze([
        { condition: "totalCollections >= 10", points: 15 },
        { condition: "totalCollections >= 5", points: 9 },
        { condition: "totalCollections < 5", points: 3 },
      ]),
    }),
    nftHoldingsScore: Object.freeze({
      maxPoints: 15,
      thresholds: Object.freeze([
        { condition: "totalNFTs >= 200", points: 15 },
        { condition: "totalNFTs >= 100", points: 11 },
        { condition: "totalNFTs < 100", points: 4 },
      ]),
    }),
    nativeBalanceScore: Object.freeze({
      maxPoints: 10,
      thresholds: Object.freeze([
        { condition: "nativeBalance >= 5", points: 10 },
        { condition: "nativeBalance >= 1", points: 7 },
        { condition: "nativeBalance < 1", points: 3 },
      ]),
    }),
    dacInceptionRankScore: Object.freeze({
      maxPoints: 30,
      thresholds: Object.freeze([
        { condition: "rankBadgeCount >= 13", inferredRank: "CROWN", points: 30 },
        { condition: "rankBadgeCount >= 12", inferredRank: "CIPHER", points: 28 },
        { condition: "rankBadgeCount >= 11", inferredRank: "PHANTOM", points: 27 },
        { condition: "rankBadgeCount >= 10", inferredRank: "INTERCEPTOR", points: 25 },
        { condition: "rankBadgeCount >= 9", inferredRank: "ARCHITECT", points: 23 },
        { condition: "rankBadgeCount >= 8", inferredRank: "WARRIOR", points: 21 },
        { condition: "rankBadgeCount >= 7", inferredRank: "SOVEREIGN", points: 19 },
        { condition: "rankBadgeCount >= 6", inferredRank: "SENTINEL", points: 16 },
        { condition: "rankBadgeCount >= 5", inferredRank: "VANGUARD", points: 13 },
        { condition: "rankBadgeCount >= 4", inferredRank: "SHADOW UNIT", points: 10 },
        { condition: "rankBadgeCount >= 3", inferredRank: "SEAL", points: 8 },
        { condition: "rankBadgeCount >= 2", inferredRank: "COMMANDO", points: 6 },
  ...
  note:
    "Community-defined scoring heuristic. DAC Inception Rank is treated as a strong participation signal, but this is not an official DAC reputation, eligibility, or Sybil system. v1.3.1 highlights inferred rank in the UI.",
});
...
function buildReputationScore({
  ...
  const transactionScore =
    txCount >= 1000 ? 30 : txCount >= 500 ? 22 : txCount >= 100 ? 15 : 7;

  const nftDiversityScore =
    totalCollections >= 10 ? 15 : totalCollections >= 5 ? 9 : 3;

  const nftHoldingsScore =
    totalNFTs >= 200 ? 15 : totalNFTs >= 100 ? 11 : 4;

  const nativeBalanceScore =
    nativeBalance >= 5 ? 10 : nativeBalance >= 1 ? 7 : 3;

  const rankScore = getRankScoreByBadgeCount(rankBadgeCount);
  const dacInceptionRankScore = rankScore.points;
  ...
  if (rankBadgeCount >= 6 && txCount > 500) {
    trustProfile = "VERIFIED INCEPTION PARTICIPANT";
  }

  if (txCount > 1000 && totalCollections > 10 && rankBadgeCount >= 6) {
...
function buildKnownCollectionSignals(nftCollections) {
  ...
  return {
    registryVersion: "known-collection-registry-v1.3.1",
    source: "Community-maintained known collection registry",
    scoringEffect: "reputation-score-component",
    note:
      "DAC Inception Rank is included as a 30-point component in the locked WIL-v1.3.1 reputation score.",
    matched,
  };
}
...
function renderKnownCollectionRegistry(registry) {
  ...
  if (el.knownRegistryStatusLabel) {
    el.knownRegistryStatusLabel.textContent = registry
      ? registry.registryVersion || "known-collection-registry-v1.3.1"
      : "Community registry v1.3.1";
  }
  ...
  if (el.rankDetectionDetail) {
    if (signal && signal.detected) {
      el.rankDetectionDetail.innerHTML =
        ...
        `<span class="rank-detected-line">Inferred rank: <span class="rank-highlight">${escapeHtml(signal.inferredRank)}</span> · Rank score: ${signal.rankScore}/30.</span>`;
    } else if (registry) {
...
function renderScoringPolicy(policy) {
  ...
  if (el.scoringPolicyLabel) {
    el.scoringPolicyLabel.textContent = safePolicy.version || "WIL-v1.3.1";
  }

  if (el.policyIdValue) {
    el.policyIdValue.textContent = safePolicy.policyId || "WIL-2026-05-v1.3.1";
  }
  ...
  if (el.policyEngineValue) {
    el.policyEngineValue.textContent = safePolicy.model || "rank-aware-reputation-scoring-v1.3.1";
  }
}
...
function renderScoringBreakdown(reputationScoring) {
  ...
  if (el.scoreExplanation) {
    el.scoreExplanation.textContent =
      "This score is generated from five transparent components under a locked scoring policy: transaction activity, NFT diversity, NFT holdings, native balance, and DAC Inception Rank. The labels and thresholds are community-defined and are not official DAC eligibility criteria.";
  }
}
