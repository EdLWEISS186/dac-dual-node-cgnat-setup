...
function buildDynamicIntelligenceBadge(output) {
  ...
  const metadata = {
    name: `${INTELLIGENCE_BADGE_NAME} — ${shortWallet}`,
    description:
      "Wallet Status badge preview generated by the DAC Wallet Intelligence Layer. Mint/update is executed directly against the Wallet Status contract, which verifies the core profile on-chain.",
    image: "data:application/json;utf8,ONCHAIN_CORE_METADATA",
...
function renderDynamicIntelligenceBadge(badge) {
  ...
  el.badgePreviewSubtitle.textContent = `One evolving badge for ${shortenAddress(badge.wallet)}.`;
  el.badgePreviewPattern.textContent =
  ...
  if (el.badgeContractValue) {
    el.badgeContractValue.textContent = shortenAddress(WALLET_STATUS_BADGE_CONTRACT);
