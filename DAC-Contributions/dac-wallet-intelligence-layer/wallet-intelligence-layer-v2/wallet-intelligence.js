...
const INTELLIGENCE_BADGE_NAME = "Wallet Status SBT";
...
async function setDynamicBadgePreviewImage(wallet) {
  if (!el.badgePreviewImage || !el.badgePreviewCard || !wallet) return;

  const requestId = ++badgePreviewImageRequestId;

  try {
    el.badgePreviewImage.removeAttribute("src");
    el.badgePreviewImage.alt = "Loading dynamic Wallet Status SBT preview...";

    const encodedWallet = encodeAddressParam(wallet);
    const result = await rpcCall("eth_call", [
      {
        to: WALLET_STATUS_BADGE_CONTRACT,
        data: WALLET_STATUS_BADGE_SELECTORS.dynamicImageOf + encodedWallet,
      },
      "latest",
    ]);
    ...
    const imageDataUri = decodeAbiString(result);

    if (!imageDataUri || !imageDataUri.startsWith("data:image/svg+xml;base64,")) {
      throw new Error("Invalid dynamic SVG image returned by Wallet Status SBT contract.");
    }

    el.badgePreviewImage.src = imageDataUri;
    el.badgePreviewImage.title = `Dynamic on-chain SVG preview for ${wallet}`;
    el.badgePreviewImage.alt = `Wallet Status SBT preview for ${wallet}`;
    el.badgePreviewCard.classList.add("has-final-art");
  } catch (error) {
    console.warn("Dynamic badge preview image failed:", error);
    if (requestId !== badgePreviewImageRequestId) return;

    // Fallback to the local HTML preview instead of showing stale artwork.
